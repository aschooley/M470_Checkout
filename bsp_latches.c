...
static void modify_bitfield (uint16_t * bitfield, uint16_t modifier, power_state_t operation);
...
void bsp_set_heater_power(thermal_zone_t zone, power_state_t state){
	...
	else if((CARD_BANK_2 == zone_address.bank) && (BOX_CARD == zone_address.box))
	{
		// Initalize memory for this latch, it will not be reinitalized since it is static.
		static uint16_t latch_state = 0;
		// Update the saved latch state with the modificaiton.
	...
	{
		logf(ERROR,"Invalid bank or box. Bank = %d Box = %d",zone_address.bank,zone_address.box);
	}
	// Do the operation.
	bsp_pin_digital_write(pin,DISABLED);
	send_address(retrieved_latch_state);
...
static void modify_bitfield (uint16_t * bitfield, uint16_t bitmask, power_state_t operation)
{
if(ON == operation)
{
	*bitfield |= bitmask;
